import { divisions, districts } from "./division";

export let postcodes = [
  {
    division_id: "1",
    district_id: "34",
    upazila: "Amtali",
...
export function findUpozella(divname, districtname) {
  let divdis = [];
  let divId, disId;
  districts.map((v) => {
    if (v.name === districtname) {
      disId = v.id;
    }
  });
  divisions.map((v) => {
  ...
  postcodes.map((v) => {
    if (v.district_id === disId && v.division_id === divId) {
      divdis.push({
        id: v.upazila,
        title: v.upazila,
        value: v.upazila,
